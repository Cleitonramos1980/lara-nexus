...
export const laraOrquestracaoMensagemBodySchema = z.object({
  ...
  tenant_id: z.string().trim().optional().default("default"),
  event_id: z.string().trim().optional().default(""),
  phoneNumberId: z.string().trim().optional().default(""),
  canal: z.enum(["WHATSAPP", "SMS", "EMAIL", "VOICE", "OUTRO"]).default("WHATSAPP"),
  jurisdicao: z.enum(["BR", "US", "EU", "UK", "GLOBAL"]).default("BR"),
  payload: z.record(z.any()).optional(),
});

export const laraOrquestracaoRespostaQuerySchema = z.object({
  protocolId: z.string().trim().optional().default(""),
  conversationId: z.string().trim().optional().default(""),
});

export const bradescoPixWebhookBodySchema = z
  .object({
    event_id: z.string().trim().optional(),
    tenant_id: z.string().trim().default("default"),
    txid: z.string().trim().optional(),
    endToEndId: z.string().trim().optional(),
    end_to_end_id: z.string().trim().optional(),
    e2eid: z.string().trim().optional(),
    valor: z.union([z.string(), z.number()]).optional(),
    horario: z.string().trim().optional(),
    chave: z.string().trim().optional(),
    pix: z.array(z.record(z.any())).optional(),
    payload: z.record(z.any()).optional(),
  })
  .passthrough();
...
export const pagamentoBolepixBodySchema = z.object({
  wa_id: z.string().trim().optional(),
  codcli: z.coerce.number().int().positive(),
  cliente: z.string().trim().optional(),
  duplicatas: z.array(z.string().trim()).optional(),
...
export const bradescoBolepixOperationBodySchema = z.object({
...
export const bradescoBolepixWebhookBodySchema = z
  .object({
    event_id: z.string().trim().optional(),
    tenant_id: z.string().trim().default("default"),
    payload: z.record(z.any()).optional(),
...
export const reguaDispararClienteBodySchema = z.object({
  codcli: z.coerce.number().int().positive(),
});

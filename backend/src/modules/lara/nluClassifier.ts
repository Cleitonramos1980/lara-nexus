...
const BASE_LEXICON: IntentLexicon = {
  ...
  solicitar_negociacao: ["negociar", "negociacao", "parcelar", "parcelamento", "desconto", "acordo", "proposta", "condicao especial", "nao tenho dinheiro", "sem condicoes", "nao consigo pagar", "desempregado", "situacao dificil", "nao tenho data", "nao sei quando", "passando por dificuldade"],
  ...
  optout: ["pare", "parar", "stop", "sair", "remover", "descadastrar", "opt out", "nao quero mensagem", "tira meu nome", "me remove", "chega de mensagem", "cancela contato"],
  optin: ["continuar", "voltar", "quero receber", "pode enviar", "reativar", "me inclua", "quero mensagem", "volta a me enviar"],
  neutro: [],
};

// Cache do léxico dinâmico — recarregado a cada LEXICON_RELOAD_MS
let _dynamicLexiconCache: Partial<IntentLexicon> = {};
let _lexiconLastLoaded = 0;
const LEXICON_RELOAD_MS = 30 * 60 * 1000; // recarrega a cada 30 min

async function loadDynamicLexicon(): Promise<void> {
  const now = Date.now();
  if (now - _lexiconLastLoaded < LEXICON_RELOAD_MS) return;
  try {
    const configs = await laraOperationalStore.listConfiguracoes();
    const cfg = configs.find((c) => c.chave === "LARA_NLU_LEXICON_OVERRIDE_JSON");
    if (cfg?.valor) {
      const parsed = JSON.parse(cfg.valor) as Partial<IntentLexicon>;
      if (parsed && typeof parsed === "object") {
        _dynamicLexiconCache = parsed;
        _lexiconLastLoaded = now;
      }
    }
  } catch {
    // fallback silencioso: mantém cache anterior
  }
}

// Retorna lexicon mesclado (base + aprendido) sem aguardar (dispara reload async)
function getLexicon(): IntentLexicon {
  // Dispara reload sem bloquear
  void loadDynamicLexicon();

  const merged = { ...BASE_LEXICON };
  for (const [intent, terms] of Object.entries(_dynamicLexiconCache)) {
    const key = intent as LaraIntent;
    if (Array.isArray(terms) && BASE_LEXICON[key]) {
      // Une termos aprendidos com base, deduplicando
      const existing = new Set(merged[key]);
      for (const t of terms) {
        if (!existing.has(t)) merged[key] = [...merged[key], t];
      }
    }
  }
  return merged;
...
// Mantém compatibilidade com código legado que usa LEXICON diretamente
const LEXICON: IntentLexicon = new Proxy(BASE_LEXICON, {
  get(target, prop) {
    const live = getLexicon();
    return live[prop as LaraIntent] ?? target[prop as LaraIntent] ?? [];
  },
});
...
async function classifyIntentWithOpenAi(messageText: string): Promise<{
  ...
  // Se reservamos o slot de probe no half_open, garante liberação mesmo em throws inesperados
  const probeReserved = openAiCircuit.state === "half_open";
  ...
  try {
    ...
    // Rede de segurança: se um throw inesperado bypassou markCircuit*, libera o probe slot
    if (probeReserved && openAiCircuit.halfOpenProbeInFlight) {
      openAiCircuit.halfOpenProbeInFlight = false;
    }
  }
}
...
export function classifyIntentWithNlu(messageText: string): NluResult {
  ...
  const scoreMap: Record<LaraIntent, number> = {
    ...
    solicitar_pagamento: scoreIntent(tokens, normalizedText, LEXICON.solicitar_pagamento),
...
/**
 * Força recarga imediata do léxico dinâmico (usado pelo learningEngine após aplicar correções).
 */
export async function reloadDynamicLexicon(): Promise<void> {
  _lexiconLastLoaded = 0; // força reload no próximo getLexicon()
  await loadDynamicLexicon();
}
